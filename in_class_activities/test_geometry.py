...
# create objects as needed in setup or other places 
    ...
    def test_002_point_equality(self):
        self.assertEqual(int(self.origin), Point(0,0,0))
        self.assertEqual(int(self.one_above_origin),Point(0, 1, 0))
        self.assertEqual(int(self.one_under_origin),Point(0, -1, 0))
        self.assertEqual(int(self.out_point),Point(0, 1000, 0))

    def test_003_sphere_area(self):
        self.assertAlmostEqual(int(self.one_radius_sphere.area()),12)
        self.assertAlmostEqual(int(self.ten_radius_sphere.area()),1256)
    
    def test_004_sphere_volume(self):
        self.assertAlmostEqual(int(self.one_radius_sphere.volume()),4)
        self.assertAlmostEqual(int(self.ten_radius_sphere.volume()),4188)

    def test_005_sphere_is_inside_point(self):
        self.one_radius_sphere.is_inside_point(self.origin)
        self.ten_radius_sphere.is_inside_point(self.origin)

    def test_006_sphere_is_inside_sphere(self):
        self.one_radius_sphere.is_inside_sphere(self.ten_radius_sphere)
        self.ten_radius_sphere.is_inside_sphere(self.one_radius_sphere)

    def test_006_cube_is_inside_point(self):
        self.one_cube.is_inside_point(self.origin)
        self.ten_cube.is_inside_point(self.origin)       

    def test_007_is_inside_sphere(self):
        self.one_cube.is_inside_sphere(self.one_radius_sphere)
        self.ten_cube.is_inside_sphere(self.ten_radius_sphere)


# Here we have the test but we do not have the implementation of the method 
        self.one_cube.is_inside_cube(self.ten_cube)
# 
